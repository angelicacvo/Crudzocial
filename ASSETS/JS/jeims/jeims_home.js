...
// Logout
document.getElementById("logout__btn")?.addEventListener("click", async () => {
    if (confirm("¿Seguro que deseas cerrar sesión?")) {
        await logAction("Cierre de sesión");
...
function renderCard({ id, image, text, userEmail, timestamp }) {
    ...
    const fecha = timestamp ? new Date(timestamp).toLocaleString() : "";
    ...
    card.innerHTML = `
        ...
        <div class="gallery-card__body">
            ...
            <small class="gallery-card__meta">Autor : ${displayName}</small>
            ${fecha ? `<small class="gallery-card__meta">${fecha}</small>` : ""}
            ${
                (isOwner || isAdmin)
